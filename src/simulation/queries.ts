...
export function getPlayerView(s: GameState): PlayerView {
    ...
    const systems = s.systems.map(sys => { const survey = Object.values(intel).find(r=>r.systemId===sys.id && r.survey)?.survey; return { id: sys.id, name: sys.name, x: sys.x, y: sys.y, z: sys.z, distance: sys.distance, spectral: sys.spectral, planets: sys.planets, bodies: sys.bodies.map(body => { const known = intel[body.id]?.survey?.bodies.find(b => b.id === body.id); const { habitability, resources, ...observed } = body; return { ...observed, ...(known ? { habitability: known.habitability, resources: known.resources } : {}) }; }), habitability: survey?.habitability ?? 0, resources: survey?.resources ?? 0, risk: survey?.risk ?? 0, ...(survey?.anomaly ? { anomaly: survey.anomaly } : {}) }; });
    return { time: s.time, seed: s.seed, systems, intel, worlds, ships: structuredClone(s.knownShips), orders: structuredClone(s.knownOrders), messages: structuredClone(s.messages), milestones: s.milestones.filter(x => !x.startsWith('founded-')), routes: [...s.routes], failed: s.failed, credits: worlds[HOME].finance.balance, capacity: 100 };
}
...
export function researchEstimate(w:WorldState,key:ResearchFocus) {
    const science=w.districts.reduce((n,d)=>n+(d.zone==='science'?1:0)*(d.progress/100+.2)*(1-d.damage/100),0);
