export type MusicInstrument = 'pad' | 'piano' | 'pluck' | 'bell' | 'bass'
    | 'drone' | 'strings' | 'cello' | 'horn' | 'flute' | 'kick' | 'snare' | 'hat';
...
function blueCradle(): readonly MusicNote[] {
    ...
    bars.forEach(([bass, low, middle, high], bar) => {
        ...
        pattern.forEach((midi, step) => {
            add(notes, 'piano', midi, start + step * eighth, 1.8 * eighth,
    ...
    melody.forEach(([bar, step, midi, length]) => {
        add(notes, 'piano', midi, (bar * 6 + step) * eighth, length * eighth,
            bar >= 24 ? .068 : .087, .24);
    });
    [48, 60, 64, 67, 72].forEach((midi, voice) => {
        add(notes, 'piano', midi, 70 + voice * .055, 4.5, .043, (voice - 2) * .12);
    });
    return byTime(notes);
}
...
function longVoyage(): readonly MusicNote[] {
    ...
    add(notes, 'piano', 62, 5.7, 4.5, .054, -.63);
    ...
    add(notes, 'piano', 65, 30.7, 4.8, .047, .5);
    add(notes, 'piano', 64, 34.25, 3.1, .030, -.25);

    add(notes, 'drone', 38, 43.2, 14.5, .080, -.1);
    add(notes, 'bell', 74, 45.9, 3.8, .032, .62);
    add(notes, 'drone', 50, 48.7, 9.4, .039, .35);
    add(notes, 'piano', 69, 55.1, 6.3, .048, -.5);
    ...
    add(notes, 'piano', 64, 75.2, 4.1, .045, .45);
    add(notes, 'drone', 38, 82, 7.5, .060, -.15);
    add(notes, 'cello', 50, 82.7, 6.5, .032, -.38);
    add(notes, 'piano', 69, 84.6, 4.2, .034, .4);
    return byTime(notes);
}
    ...
    harmony.forEach(([bass, middle, upper], bar) => {
        ...
        if (interlude) {
            [middle, upper, middle + 12].forEach((midi, step) =>
                add(notes, 'piano', midi, start + (step + .5) * beat, 1.6 * beat, .045, .3));
        }
        if (bar >= 24) {
...
export const MUSIC_TRACKS: readonly MusicTrack[] = [
    ...
    {
        id: 'blue-cradle', title: '蓝色摇篮', description: '六拍钢琴与大提琴问答，越过小调桥段后温柔归航',
    ...
    {
        id: 'delayed-echoes', title: '迟来的回声', description: '钟声问答与三音拨弦交织，主题拉长后反向回应',
        duration: 84, space: { decay: 1.4, mix: .16 }, notes: delayedEchoes(),
    },
    {
        id: 'dawn-beyond', title: '彼岸晨曦', description: '弦乐与长笛走过静谧钢琴间奏，在圆号对位中迎来晨光',
